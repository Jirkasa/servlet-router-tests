...
public class DisplayBaseURLController extends HttpController {
	@Override
	protected void handleGet(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try (PrintWriter out = response.getWriter()) {
			...
			out.println("<title>BASE URL: " + request.getAttribute("BASE_URL") + "</title>");
			out.println("</head>");
			out.println("<body>");
			out.println("<h1>BASE URL: " + request.getAttribute("BASE_URL") + "</h1>");
			out.println("</body>");
			out.println("</html>");
		}
	}
